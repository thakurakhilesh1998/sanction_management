...
<div class="card m-4">
    ...
    <div class="card-body">
        ...
        @endif
        <h4 class="p-2">RGSA Sanctions for the Gram Panchayat
            {{-- {{$sanction[0]->progress_csc->count()}}
            {{$sanction[0]->progress_csc->completion_percentage}} --}}
            @if(!isset($cscSan[0]->progress_csc) || optional($cscSan[0]->progress_csc)->completion_percentage === '-1')
                <a href="{{url("xen/add-progress-csc".'/'.$cscSan[0]->gp.'/'.$cscSan[0]->block.'/'.$cscSan[0]->district.'/'.$cscSan[0]->work)}}" class="btn btn-primary btn-sm float-right">Add Progress</a>
            @elseif(!isset($cscSan[0]->progress_csc) || optional($cscSan[0]->progress_csc)->completion_percentage === 'Work Completed')    
                <a href="{{url("xen/update-progress-csc".'/'.$cscSan[0]->gp.'/'.$cscSan[0]->block.'/'.$cscSan[0]->district.'/'.$cscSan[0]->work)}}" class="btn btn-primary btn-sm float-right">View Progress Added</a> 
            @else
                <a href="{{url("xen/update-progress-csc".'/'.$cscSan[0]->gp.'/'.$cscSan[0]->block.'/'.$cscSan[0]->district.'/'.$cscSan[0]->work)}}" class="btn btn-primary btn-sm float-right">Update Progress</a> 
            @endif  
        </h4>
        @if($cscSan->isEmpty())
                ...
                <table class="table table-bordered text-center" id="datatable">
                    ...
                    @foreach ($cscSan as $san)
                        <tr>
                            ...
                            <td>
                                @if($san->uc==null)
                                    @if($san->progress_csc!==null)
                                        @if($san->progress_csc->completion_percentage==='Work Started' || $san->progress_csc->completion_percentage==='Partial Completion' || $san->progress_csc->completion_percentage==='Work Completed')
