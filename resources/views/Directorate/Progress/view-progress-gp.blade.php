...
<div class="card m-4">
    ...
    <div class="card-header">
        <h3 class="h3 mb-3 text-gray-800">View Progress of Sanctions of <strong>{{$block}}</strong> 
        </h3>
    </div>
    <div class="card-body">
            ...
            <table class="table table-bordered text-center table-striped" id="districtTable">
                ...
                <tbody>
                        @php
                            ...
                            $totalSan=0;
                        @endphp
                        @foreach ($gpSanction as $index => $san)
                            <tr>
                                <td>{{$index+1}}</td>
                                <td>{{$san->gp}}</td>
                                <td><a href="{{url('dir/viewGpDetails'.'/'.$san->gp.'/'.$block)}}">{{$san->total_sanctions}}</a></td>
