    ...
    <div class="card-body">
        ...
        <h4 class="p-2">Sanctions for the Gram Panchayat 
            ...
            {{-- @if(!isset($sanction[0]->progress) || $sanction[0]->progress->completion_percentage=='-1') --}}
                <a href="{{url("gp/add-progress".'/'.$sanction[0]->gp.'/'.$sanction[0]->block.'/'.$sanction[0]->district)}}" class="btn btn-primary btn-sm float-right">Add Progress</a>
            @else
                <a href="{{url("gp/update-progress".'/'.$sanction[0]->gp.'/'.$sanction[0]->block.'/'.$sanction[0]->district)}}" class="btn btn-primary btn-sm float-right">Update Progress</a> 
            @endif 
        </h4>
        @if($sanction->isEmpty())
                ...
                <table class="table table-bordered text-center" >
                    ...
                    @foreach ($sanction as $san)
                        <tr>
                            ...
                            <td>
                                <a href="{{ url('gp/view-sanction-file', ['filename' => $san->san_sign_pdf]) }}" target="_blank">View Sanction File</a>
                            </td>
                            <td>
                                @if($san->uc==null)
                                    @if($san->progress!==null)
                                        @if($san->progress->completion_percentage==='Work Started' || $san->progress->completion_percentage==='Partial Completion' || $san->progress->completion_percentage==='Work Completed')
                                            <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#uploadUc"  data-sanction-id="{{$san->id}}">Upload UC</button></td>
                                        @else
                                    ...
                                    @endif
                                @else
                                    <a href="{{url('gp/viewUCgp',['filename'=>$san->uc])}}" target="_blank">View UC file</a>
                                 @endif
                            </td>
                        </tr>
                    @endforeach
