...
function validateForm()
{
    ...
    // Validate Sanction File
    if (file) {
        // Check type
        if (file.type !== "application/pdf") {
            isValid=false;
            $('#san_sign_pdf').next(".error").remove();
            $('#san_sign_pdf').after("<div class=' mt-2 alert alert-danger error'>Please select PDF file only.</div>");
        }

        // Check size (1 MB = 1*1024*1024 bytes)
        if (file.size > 1 * 1024 * 1024) {
            isValid=false;
            $('#san_sign_pdf').next(".error").remove();
            $('#san_sign_pdf').after("<div class=' mt-2 alert alert-danger error'>File size should not be more than 1 MB.</div>");
        }
    } else {
            isValid=false;
            $('#san_sign_pdf').next(".error").remove();
            $('#san_sign_pdf').after("<div class=' mt-2 alert alert-danger error'>Please select a PDF file.</div>");
    }
    return isValid;
}